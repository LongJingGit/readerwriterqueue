...
namespace moodycamel
{
    ...
    // compiler fence
    // std::atomic_signal_fence  仅在信号处理程序和在同一线程上运行的其他代码之间执行同步
    // 也可以理解成：在同一线程中执行的线程与信号处理函数之间的隔离
    AE_FORCEINLINE void compiler_fence(memory_order order) AE_NO_TSAN
    {
        switch (order)
        {
...
namespace moodycamel
{
    ...
    public:
        ...
        {
            int res = value.fetch_add(increment, std::memory_order_acquire);
            return res;
        }
...
#error Unsupported platform! (No semaphore wrapper available)
        ...
        {
        public:
            // 通过保留任何的 cv 限定符来获取与T(Triat类)相应的带符号类型
        ...
        private:
            ...
            bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1) AE_NO_TSAN
            {
                ...
                while (--spin >= 0)
                {
                    if (m_count.load() > 0)
                    {
                        // 如果在自旋等待期间有元素入队或者出队，则更新计数
                        m_count.fetch_add_acquire(-1);
                        return true;
                    }
                    compiler_fence(memory_order_acquire); // Prevent the compiler from collapsing the loop.
                }
                // 减一更新计数（这里假设的是有线程在自旋结束之后对队列进行了入队或者出队操作，并且更新了计数）
                // 如果没有线程更新计数，则这里的 m_count.load() == -1（在这之前 m_count.load() == 0）
                oldCount = m_count.fetch_add_acquire(-1);
                if (oldCount > 0)
                    return true;
                // 如果没有设置超时时间，就在这里死等信号量
                if (timeout_usecs < 0)
                {
                    if (m_sema.wait())
                        return true;
                }
                // 设置了超时时间，且在超时时间内等到了信号量
                ...
                // 设置了超时等待时间，但是仍然没有等到信号量
                // 但是计数仍然在减少，表明我们仍然需要等待。在信号量没有发出足够信号的情况下，我们必须重新调整计数（释放信号量）
                // 如果设置了超时时间，且在上面的步骤都没有等到信号量，则在这里一直死等信号量
                while (true)
                {
                    // 在上面已经将 m_count 更新成了 -1，所以这里需要还原
                    oldCount = m_count.fetch_add_release(1);
                    // 假设在上面将计数更新为 -1 之后一直没有生产者线程进行入队操作，并且没有更新计数，那么 oldCount 就为 -1，返回 false（并且需要将 m_count 的计数还原）
                    if (oldCount < 0)
                        return false; // successfully restored things to the way they were

                    // 但是还有一种情况就是在上面将 m_count 更新为 -1 到此处的 while 循环期间，有生产者线程进行了入队操作，所以 oldCount 就不是 -1
                    // 所在在使用 add_release(1) 将计数还原之后，再进行减一操作更新计数
